...
fun buildList(
    ...
    val initials = listInitials(names)
    val prompts = getRecord(records)

    fun initFunction(it: Int): String {
        if (initials.isEmpty() && prompts.isEmpty()) {
            return ""
        }
        return "${initials.getOrElse(it){nameUnknown}} ${prompts.getOrElse(it){recordUnknown}}"
    }
...
fun listInitials(names: String): List<String> {
    val listNames = fileReadAsList(names)
    return listNames.map(::getInitials)
}

// will break each full name into initials
fun getInitials(fullName: String): String {
    val firstLast = fullName.split(" ")
    return "${firstLast[0].first()}${firstLast[1].first()}"
}

// Uses getPrompt and getrecordRatio to create list of win prompts
fun getRecord(records: String): List<String> {
    val listRecords = fileReadAsList(records)
    val listRatios = listRecords.map(::getRecordRatio)
    return listRatios.map(::getPrompt)
}

// determines whether you have more wins or losses
fun getRecordRatio(record: String): String {
    val recordList = record.split(",")
    val wins = recordList.filter { it == "W" }
    return "${recordList.size - wins.size * 2}"
}

// turns win loss ratio into corresponding prompt
fun getPrompt(winLoss: String): String {
    return when (winLoss.toInt() > 0) {
        false -> recordWin
        else -> recordLoss
    }
}
